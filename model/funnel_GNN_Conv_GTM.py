...
class GNN_Conv_GTM(torch.nn.Module):
    ...
    def __init__(self, in_channels, out_channels, n_class=2, gtm_grid_dims=(10, 10), rbf=10, sigma=1, gtm_lr=1e-3,
        ...
        self.gtm2 = GTM(out_channels * 2, out_size=(gtm_grid_dims[0],gtm_grid_dims[1]), m=rbf, sigma=sigma, gtm_lr=gtm_lr, method='full_prob', learning=learning, device=self.device)
        self.gtm3 = GTM(out_channels * 3, out_size=(gtm_grid_dims[0],gtm_grid_dims[1]), m=rbf, sigma=sigma, gtm_lr=gtm_lr, method='full_prob', learning=learning, device=self.device)
