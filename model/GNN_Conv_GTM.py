...
class GNN_Conv_GTM(torch.nn.Module):
    def __init__(self, in_channels, out_channels, n_class=2, gtm_grid_dims=(10, 10), dropout=0, device=None):
        ...
        self.gtm1 = GTM(out_channels, out_size=gtm_grid_dims, device=self.device)
        self.gtm2 = GTM(out_channels * 2, out_size=gtm_grid_dims, device=self.device)
        self.gtm3 = GTM(out_channels * 3, out_size=gtm_grid_dims, device=self.device)

        # define  read_out # TODO fix hardcoded dimension from GTM output (coming from MatM)
        self.out_conv1 = GraphConv(2, self.out_channels)
        self.out_conv2 = GraphConv(2, self.out_channels)
        self.out_conv3 = GraphConv(2, self.out_channels)
