...
class GTM(nn.Module):
    ...
    def transform(self, Xt):
        ...
        with torch.no_grad():
            if self.method == 'full_prob':
                return self.responsibility(Xt).T # this is equal to self.R_inc
    ...
    def forward(self, t, sigma=None):
        ...
        return -self.log_likelihood(t).item(), GTM_output
    ...
    def train_aggregator(self, x, current_iter, max_iter, lr, first, second):
        ...
        if self.learning == 'standard':
            ...
            return -self.log_likelihood(x).item()
            ...
            if self.second_old > second:  # new epoch, perform M-step
                ...
                self.beta = self.X_inc.numel() / torch.cdist(self.phi.matmul(self.W), self.X_inc, p=2,
                                                             compute_mode='donot_use_mm_for_euclid_dist').pow_(2).mul_(
                    self.R_inc).sum()
                self.second_old = 0
            ...
            return -self.log_likelihood(self.X_inc).item()  # - due to MINIMIZATION
