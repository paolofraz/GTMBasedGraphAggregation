...
def load_file(result_folder, test_name, n_split, suffix=""):
    ...
    # for i in [0,1,3,4,5,6,7,8,9]:
        test_res_from_file=np.loadtxt(os.path.join(result_folder,suffix+test_name+"--split-"+str(i)+"_test"), skiprows=2)
        train_res_from_file=np.loadtxt(os.path.join(result_folder,suffix+test_name+"--split-"+str(i)+"_train"), skiprows=2)
        valid_res_from_file=np.loadtxt(os.path.join(result_folder,suffix+test_name+"--split-"+str(i)+"_valid"), skiprows=2)
...
def perform_validation_loss(log_file, split_result, n_epoch, test_epoch):
    ...
    for i,split in enumerate(split_result):
        ...
        #get loss column
        loss_list=valid_res[0:n_epoch,2] #NOTE:validation based on loss value, if it has to be computed on acc change index with 3, and use argmax 2 rows bellow
